...
public class Transaction {
    ...
    static Transaction[] createTransaction(User sender, User recipient, Integer amount) {
        ...
        tr[1] = new Transaction(id, sender, recipient, TransferCategory.DEBIT, amount);
    ...
    public static String getHeader() {
        return String.format("%-36s %-13s %-13s %-10s %10s",
                "ID", "Sender(ID)", "Recipient(ID)", "Category", "Amount");
    }

    @Override
    public String toString() {
        return String.format("%-36s %-13d %-13d %-10s %10s",
                id, sender.getId(), recipient.getId(), category, amount);
    }
}
