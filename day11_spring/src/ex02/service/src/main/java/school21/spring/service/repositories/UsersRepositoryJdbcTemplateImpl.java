    ...
    public void save(User entity) {
        ...
        jdbcTemplate.update(connection -> {
                    PreparedStatement statement =
                            connection.prepareStatement("insert into users (email, password) values (?, ?)", new String[]{"id"});
    ...
    public Optional<User> findByEmail(String email) {
        try {
            List<User> users = jdbcTemplate.query("select * from users where email = ?",
                    (rs, dw) -> new User(rs.getLong("id"), rs.getString("email"), rs.getString("password")),
                    email);
